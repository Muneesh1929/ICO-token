  ...
  return <>
    {
      ...
      ) : (<Input placeholder={"Token A"} handleClick={(e) => setToken_A(e.target.value)} />
      )}
       {
      token_2 ? (
        <Input value={`${token_2?.name} (${token_2?.symbol}) Bal: ${token_2?.balance.slice(0, 8)}`} />
      ) : (<Input placeholder={"Token B"} handleClick={(e) => setToken_B(e.target.value)} />
      )}

<Input placeholder={"Fee"} handleClick={(e) => setFee(e.target.value)} />

<button onClick={() => CALLING_POOL_Add()} className="btn btn--large btn--green-light btn--with-icon btn--icon-right full-width">Check Pool
